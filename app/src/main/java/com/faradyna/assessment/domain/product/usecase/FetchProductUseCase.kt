...
): BaseSuspendUseCase<FetchProductUseCase.RequestValues, FetchProductUseCase.ResponseValues>() {

    class RequestValues(val request: Int) : BaseSuspendUseCase.RequestValues

    class ResponseValues(val result: ResultState<ProductDomain?>) : BaseSuspendUseCase.ResponseValues

    override fun execute(requestValues: RequestValues): Flow<ResponseValues> {
        return flow {
            emit(ResponseValues(ResultState.Loading))
            try {
                val data = repository.fetchProduct(requestValues.request)
